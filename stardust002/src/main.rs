...
mod keygen {
    ...
    #[derive(Parser, Debug)]
    #[command(about = "Deterministic key generator: password → reproducible keystream")]
    pub struct Args {
...
mod xor {
    ...
    #[derive(Parser, Debug)]
    #[command(about = "High-reliability streaming XOR with atomic commit and optional verification")]
    pub struct Args {
        /// Input file to XOR
        #[arg(index = 1)]
        pub input: String,

        /// Key file (must be >= input size)
        #[arg(index = 2)]
        pub key: String,
        ...
        /// Verify by re-XORing output with key and comparing to input
        #[arg(long)]
        pub verify: bool,
    }
...
#[derive(Subcommand)]
enum Commands {
    /// Generate deterministic keystream from password (up to 20 GiB)
    Keygen(keygen::Args),

    /// High-reliability streaming XOR (Linux only)
    Xor(xor::Args),
}
